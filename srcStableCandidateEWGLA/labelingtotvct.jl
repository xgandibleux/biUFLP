...
=#
    ...
    labeling = Array{Vector{Vector{Int64}}}(undef,data.nJ,2)
    nlabelslayer = zeros(Int64,10)

    #predeflabels = Array{Vector{Vector{Int64}}}(undef,1000)
...
return nothing 
    ...
    for i in 2:data.nI
        ...
        # prepare the layer for the user i in filling the structure with [[b.yN[1],b.yN[2]]]
        for j in 1:data.nJ
            labeling[j,layer(i)] = [[b.yN[1],b.yN[2]]] #[[0,0]]
        end
        ...
        for (pos,jPrec) in enumerate( J1nd[ i-1 ] )
            ...
            #println(labeling[jPrec,layer(i-1)])

            if labeling[jPrec,layer(i-1)] != [[b.yN[1],b.yN[2]]] #[[0,0]]
                ...
                for (pos,j) in enumerate(J1nd[i])
                    ...
                    if addNewLabel
                        ...
                        for (pos,jND) in enumerate(J1nd[i])
                            ...
                            if labeling[jND,layer(i)] != [[b.yN[1],b.yN[2]]] #[0,0]
                    ...
                    if addNewLabel               
                        if labeling[j,layer(i)] != [[b.yN[1],b.yN[2]]] #[[0,0]]
    ...
    for (pos,jND) in enumerate(J1nd[data.nI])
        ...
        if labeling[jND,layer(data.nI)] != [[b.yN[1],b.yN[2]]] #[[0,0]]
