...
function labelingOneBox!(data::Instance, b::Box, all_YN)
    ...
    for i in 1:data.nI
        if length(vctNDalloc[i])==1
            println("num user: $i   #J1 unique: $(vctNDalloc[i])")
        end
    end
    ...
    for i in 2:data.nI
        ...
        for j in keys(labeling2[i-1])
            ...
            println("  i(previous)=$(i-1) j=$j ")

            for l in values(labeling2[i-1][j])
                # forall existing labels --------------------------------------

                println("      label to propagate : ", l.CA, " ", l.listJ1); 
                ...
                for (pos,j) in enumerate(J1nd[i])
                    ...
                    Cnew = copy(l.CA)+C
                    println("      >>>  allocation cost of i=$i at j=$j : ($(C[1]) $(C[2]))$j  --> giving [$(Cnew[1]) $(Cnew[2])]")
                    ...
                    if  (    (b.CR[2] + Cnew[2] > b.y12[2]) # out of the box at NW
                        ...
                        println("test 1 true: OUT OF BOX")
                        addNewLabel = false
                       end
                    ...
                    if addNewLabel
                        ...
                        for jND in keys(labeling2[i])
                           ...
                           !addNewLabel && break # dirty way to leave the 'for jND loop' when the new label is dominated
                            ...
                            println("               listCA :", labeling2[i][jND])
                            println("               TEST 1 $jND (isDominates(x.CA,Cnew)?) : ",any(x -> isDominates(x.CA,Cnew), labeling2[i][jND]) )
                            ...
                            if addNewLabel != false
                                ...
                                println("               TEST 2 $jND (isDominates(Cnew,x.CA)?) : ",any(x -> isDominates(Cnew,x.CA,), labeling2[i][jND]))
                    ...
                    if addNewLabel               
                        ...
                        println("           DISCARD (new label is dominated) : ",Cnew) 
                        #@assert false "stop"
                    end
    ...
    for jND in keys(labeling2[data.nI])
        ...
        for ilabel in 1:length(labeling2[data.nI][jND])
            println(labeling2[data.nI][jND][ilabel])
