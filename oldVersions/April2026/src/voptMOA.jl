...
"""
    ...
    Compute Y_N with a direct ε-constraint method (no MOA).
"""
#=
function epsilonConstraint(data::Instance, mipOptimizerToUse::String)
    ...
    if !is_solved_and_feasible(mod)   # infeasible at step 1
        return [], round(time() - getTime, digits=4)
    end

    z2_min = round(Int, value(mod[:f2]))
    push!(Y_N, [round(Int, value(mod[:f1])), z2_min])
    ...
    if !is_solved_and_feasible(mod)   # infeasible at step 2
        return [], round(time() - getTime, digits=4)
    end
    ...
    # ------------------------------------------------------------------
    # STEP 3 — main loop: min f1  s.t.  f2 <= ε, ε décroissant
    # ------------------------------------------------------------------
    eps_con = @constraint(mod, mod[:f2] <= z2_max - 1)   
    epsilon = z2_max - 1

    while epsilon >= z2_min && (time() - getTime) < time_limit

        set_normalized_rhs(eps_con, epsilon)              # update RHS
    ...
    # ------------------------------------------------------------------
    # Post-traitement : filtrage des points dominés
    # ------------------------------------------------------------------
    sort!(Y_N, by = p -> (p[1], p[2]))
    unique_f1 = Dict{Int,Int}()
    for p in Y_N
        if !haskey(unique_f1, p[1])
            unique_f1[p[1]] = p[2]
        end
    end
    Y_N = [[f1, f2] for (f1, f2) in sort(collect(unique_f1), by = x -> x[1])]

    pareto = Vector{Int64}[]
    for p in Y_N
        if isempty(pareto) || p[2] < pareto[end][2]
            push!(pareto, p)
        end
    end
    Y_N = pareto
    ...
    if elapsedTime < time_limit 
        #println(result_count(mod), " | ", elapsedTime)
    else
        Y_N = []
        #println(" -1", " | ", elapsedTime)
    end
...
for i in 1:length(YT3)
    println(i, "  ", YT1[i], "  ", YT2[i], "  ", YT3[i])
    if (YT1[i][1] != YT2[i][1]) || (YT1[i][2] != YT2[i][2]) ||
       (YT1[i][1] != YT3[i][1]) || (YT1[i][2] != YT3[i][2]) ||
       (YT2[i][1] != YT3[i][1]) || (YT2[i][2] != YT3[i][2])
       j=i+1
       println("")
       println(j, "  ", YT1[j], "  ", YT2[j], "  ", YT3[j])
       j=j+1
       println(j, "  ", YT1[j], "  ", YT2[j], "  ", YT3[j])       
       break
    end
