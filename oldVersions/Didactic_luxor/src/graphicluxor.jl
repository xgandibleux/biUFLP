...
# Size in pixels (width = height) of the square drawing area. Has no effect on
# the algorithm itself, only on this graphical output. Kept local to this file
# (self-contained) rather than in datastru.jl, since this backend is dedicated
# to the didactic instance (see main_didactic.jl) and is not part of the main
# public solver (main.jl uses graphicpyplot.jl instead).
