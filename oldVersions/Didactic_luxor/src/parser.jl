...
function getfnames(fdirectory::String)
    ...
    # setup the current directory to the targeted directory, and guarantee it
    # is restored even if an error occurs while listing the directory
    # (e.g. the directory does not exist, or a permission error).
    fnames = try
        ...
        # restore the original pwd, whether or not the block above succeeded
        cd(pwdSaved)
    end
...
function load2UFLP(fdirectory::String, fname::String)
    ...
    # `open(...) do f ... end` guarantees the file is closed even if a
    # malformed data file (e.g. a line with fewer tokens than expected) makes
    # `parse` throw partway through reading.
    return open(joinpath(fdirectory, fname)) do f
        ...
        Instance(fname,nI,nJ,c1,c2,r1,r2)
    end
