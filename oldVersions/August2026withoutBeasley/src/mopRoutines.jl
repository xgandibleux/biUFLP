...
"""
    ...
    SAFETY NOTE: this function uses `@inbounds` on yA[1], yA[2], yB[1], yB[2],
    which disables Julia's bounds checking. It assumes both vectors have at
    least 2 elements — true for every call site in this codebase (bi-objective
    points), but not enforced by the function's signature. If ever called with
    a shorter vector (e.g. during an extension to more than 2 objectives), this
    would silently read out-of-bounds memory (undefined behavior) instead of
    raising a clear BoundsError. Kept as-is deliberately: this is one of the
    most frequently called functions in the branch-and-bound pruning tests
    (evaluateTest1/2/3, tryToReducePruneBox!), so the bounds-check removal has
    a real (if small) performance benefit. Revisit if the code is ever
    generalized beyond 2 objectives, or during a dedicated performance review.
"""
