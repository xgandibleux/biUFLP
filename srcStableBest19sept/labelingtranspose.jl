...
function labelingOneBox!(data::Instance, b::Box, all_YN)
    ...
    labeling = Array{Union{Nothing,Vector{Vector{Int64}}}}(nothing,2,data.nJ)
    ...
    for (pos,j) in enumerate(J1nd[i])
        labeling[layer(i),j] = [ [ data.c1[i,j] , data.c2[i,j] ] ]
    ...
    for i in 2:data.nI
        ...
        for (pos,jPrec) in enumerate( J1nd[ i-1 ] )
            ...
            for (pos,label) in enumerate( labeling[layer(i-1),jPrec] )
                ...
                for (pos,j) in enumerate(J1nd[i])
                    ...
                    if addNewLabel
                        ...
                        for (pos,jND) in enumerate(J1nd[i])
                            ...
                            if labeling[layer(i),jND] != nothing
                                ...
                                verboseDev2 ? println("               listCA :", labeling[layer(i),jND]) : nothing
                                verboseDev2 ? println("               TEST 1 $jND (isDominates(x.CA,Cnew)?) : ",any(x -> isDominates(x,Cnew), labeling[layer(i),jND] )) : nothing

                                addNewLabel = !any(x -> isDominates(x,Cnew), labeling[layer(i),jND])
                          
                                if addNewLabel != false
                                    # test : remove all labels of 'labeling[jND,layer(i)]' dominated by 'Cnew'
                                    verboseDev2 ? println("               TEST 2 $jND (isDominates(Cnew,x.CA)?) : ",any(x -> isDominates(Cnew,x), labeling[layer(i),jND])) : nothing
                                    filter!(x -> !isDominates(Cnew,x), labeling[layer(i),jND] )
                                end
    ...
    for (pos,jND) in enumerate(J1nd[data.nI])
        ...
        if labeling[layer(data.nI),jND] != nothing

        for ilabel in 1:length(labeling[layer(data.nI),jND])
            verboseDev2 ? println(labeling[layer(data.nI)][ilabel],jND) : nothing
            verboseDev2 ? println(labeling[layer(data.nI),jND][ilabel]+b.CR) : nothing
            push!(all_YN, (b.CR[1]+labeling[layer(data.nI),jND][ilabel][1], b.CR[2]+labeling[layer(data.nI),jND][ilabel][2]) )
